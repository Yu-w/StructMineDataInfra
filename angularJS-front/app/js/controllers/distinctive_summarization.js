    ...
    $scope.getSampleQuery = function () {
        $uibModal.open({
            ...
            controller: function ($uibModalInstance, $http) {
                var $ctrl = this;
                $http.get("http://192.17.58.208:8009/distinctive_summarization/get_sample").then(function(response) {
                // $http.get("http://localhost:8009/distinctive_summarization/get_sample").then(function(response) {
                    $ctrl.request = response.data;
                    $scope.raw_query = response.data; // save the query data
                });

                $ctrl.ok = function () {
                    // $http.get("http://localhost:8009/distinctive_summarization", {params: $ctrl.request}).then(function(response) {
                    $http.get("http://192.17.58.208:8009/distinctive_summarization", {params: $ctrl.request}).then(function(response) {
    ...
    $scope.submitForm = function() {
        $uibModal.open({
            ...
            controller: function ($uibModalInstance, request, $http) {
                ...
                $ctrl.ok = function () {
                    $http.get("http://192.17.58.208:8009/distinctive_summarization", {params: request}).then(function(response) {
                    // $http.get("http://localhost:8009/distinctive_summarization", {params: request}).then(function(response) {
                        $scope.diseases = response.data;
                    });
                    $uibModalInstance.close();
                };
