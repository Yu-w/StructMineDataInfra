...
exports.detectUnsupportedNaming = exports.rewriteDestructuredTArgument = undefined;
...
function renameTestFunctionArgument(j, path, newArgument) {
    ...
    if (lastArg.type === 'ArrowFunctionExpression') {
        const arrowFunction = j.arrowFunctionExpression([j.identifier(newArgument === '' ? '()' : newArgument)], lastArg.body, false);
...
function rewriteEndCallback(j, path) {
    // calls to t.end()
    const containsCalls = j(path).find(j.CallExpression, {
        callee: {
            object: { name: 't' },
            property: { name: 'end' }
        }
    ...
    // references to t.end
    const containsReference = j(path).find(j.MemberExpression, {
        object: { name: 't' },
        property: { name: 'end' }
    }).replaceWith(j.identifier('done')).size() > 0;
...
function rewriteDestructuredTArgument(fileInfo, j, ast, testFunctionName) {
    ast.find(j.CallExpression, {
        callee: _callee => _callee.name === testFunctionName || _callee.object && _callee.object.name === testFunctionName
    }).forEach(p => {
...
function detectUnsupportedNaming(fileInfo, j, ast, testFunctionName) {
    ast.find(j.CallExpression, {
        callee: _callee2 => _callee2.name === testFunctionName || _callee2.object && _callee2.object.name === testFunctionName
    }).forEach(p => {
        const lastArg = p.value.arguments[p.value.arguments.length - 1];
        if (lastArg && lastArg.params && lastArg.params[0]) {
            ...
            if (lastArgName !== 't') {
                (0, _logger2.default)(fileInfo, `Argument to test function should be named "t" not "${lastArgName}"`, p);
            }
        }
    });
}
